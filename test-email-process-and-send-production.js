...
async function testProcessAndSendProduction() {
  console.log('🚀 Testando nova rota em PRODUÇÃO: GET /email/process-and-send\n');

  // 1. Verificar saúde da aplicação
  console.log('1️⃣ Verificando saúde da aplicação...');
  const healthResult = await makeRequest(`${BASE_URL}/health`);
  
  if (!healthResult.ok) {
    console.error('❌ Aplicação não está respondendo:', healthResult.error || healthResult.data);
    return;
  }
  
  console.log('✅ Aplicação está funcionando\n');

  // 2. Executar processamento e envio
  console.log('2️⃣ Executando processamento e envio automático...');
...
// Executar teste
testProcessAndSendProduction().catch(console.error); 